...
def generate(ext_file, output_dir):
    ...
    try:
        pip_cmd = [sys.executable, '-m', 'pip', 'install', '--target', os.path.join(temp_extension_dir, 'extension'),
                   ext_file, '--disable-pip-version-check', '--no-cache-dir', '--extra-index-url=https://azuremlsdktestpypi.azureedge.net/sdk-cli-v2']
        print_status('Executing "{}"'.format(' '.join(pip_cmd)))
        check_call(pip_cmd)
        sphinx_cmd = ['sphinx-build', '-b', 'html', os.path.dirname(os.path.realpath(__file__)), output_dir]
