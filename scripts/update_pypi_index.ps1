...
function Get-Blobs($storage, $suffix)
{
    ...
    {
        $supress = Set-AzureSubscription -SubscriptionName $storage.subscription -ErrorAction SilentlyContinue
        $supress = Set-AzureRmCurrentStorageAccount -Name $storage.account -ResourceGroupName $storage.resource
    }
    else
    {
        $supress = Set-AzureSubscription -CurrentStorageAccountName $storage.account -SubscriptionName $storage.subscription
    }
    ...
    do
    {
        $__Blobs = Get-AzureStorageBlob -Container $storage.container -MaxCount $__MaxReturn  -Prefix $prefix -ContinuationToken $__Token | Where-Object { ($_.ICloudBlob.Name.EndsWith($suffix)) -and (-not ($_.ICloudBlob.IsSnapshot)) }
...
function Create-Index($target, $storages, $external)
{
    ...
    foreach ($storage in $storages)
    {
        ...
        foreach ($_blob in $__blobs)
        {
            ...
            if ($target.cdnlink -eq $Null)
            {
                ...
                {
                    $__token = New-AzureStorageBlobSASToken -Container $storage.container -Blob $_blob.ICloudBlob.Name -Permission rl -StartTime (get-date) -ExpiryTime (get-date).AddYears(1)
                }
    ...
    Write-Host $([string]::Format("Create-Index:: Found {0} unigue package names", $__packages.Count))
    if ($local_path)
    {
    ...
    "<html><head><title>Simple Index</title><meta name='api-version' value='2' /></head><body>" >> $__index_main
    foreach ($_package in $__packages.GetEnumerator())
    {
        ...
        [string]::Format("<a href='https://{0}/{2}{1}'>{1}</a></br>", $endpoint, $__normname, $prefix) >> $__index_main
        ...
        foreach ($_item in $_package.Value.GetEnumerator() | Sort Value -Descending)
        {
            [string]::Format("<a href='{0}' rel='external'>{1}</a><br/>", $_item.Value, $_item.Key) >> $__index_item
        }
        UnIndent
    }
    ...
    if ($external -ne $Null)
    {
        foreach ($member in $external.PSObject.Properties)
        {
            ...
            [string]::Format("<a href='https://{0}/{2}{1}'>{1}</a></br>", $endpoint, $__pkgname, $prefix) >> $__index_main
        }
    }

    "</body></html>" >> $__index_main
...
function Refresh-Index($path, $storage)
{
    ...
    foreach ($_file in $__files)
    {
        ...
        $supress = Set-AzureStorageBlobContent -File $_file.FullName -Container $storage.container -Blob $__blob -Force -Properties $__blobProperties
    }
