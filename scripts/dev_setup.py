...
def setup_sdk(args: Namespace, command_args: dict) -> None:
    ...
    run_command([
        ...
        str(Path("./autorest/readme.md").absolute()), '--modelerfour.lenient-model-deduplication',
...
def setup_cli(args: Namespace, command_args: dict) -> None:
    ...
    run_command([
        ...
        f"--azure-cli-extension-folder={cli_folder.absolute()}", str(Path('./autorest/readme.md').absolute()),
...
setup sdk, cli with debug and verbose mode:                 {Color.BLUE} python scripts/dev_setup -scdv {Color.END}
    ...
    parser.add_argument('-d', '--debug', action='store_true', required=False, help='the flag to turn on debug mode for cli')
    parser.add_argument('-v', '--verbose', action='store_true', required=False, help='turn on verbose output')
    parser.add_argument('-n', '--no-install', action='store_true', required=False, help='if set, sdk and cli will not run `pip install -e package`')
    parser.add_argument('-e', '--e2etest', action='store_true', required=False, help='run the e2e tests')
    parser.add_argument('-w', '--wheel', action='store_true', required=False, help='build the wheel for package azure-ml.')
    ...
    if args.sdk:
        ...
        run_command(commands=['pip', 'install', '-r', str(Path("./tests/test-requirements.txt").absolute())],
                    **command_args)
        if system() == "Windows":
    ...
    if args.e2etest:
        ...
        run_command(commands=['pytest', str(Path("tests").absolute()), '-m', 'e2etest', '-n', '10'], shell=command_args['shell'])
    if args.wheel:
        print_red("- Build wheels for the SDK and CLi")
        azure_ml_path = Path('./src/azure-ml/setup.py')
        cli_path = Path('./src/cli/src/machinelearningservices/setup.py')
        if azure_ml_path.is_file() and cli_path.is_file():
            run_command(commands=['python', 'setup.py', 'bdist_wheel'], cwd=str(azure_ml_path.parent.absolute()), **command_args)
            run_command(commands=['python', 'setup.py', 'bdist_wheel'], cwd=str(cli_path.parent.absolute()), **command_args)
        else:
            raise FileNotFoundError("cannot find the setup.py. Make sure you in the root directory and setup sdk and cli properly")
    if args.clean:
