...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', required=True, help='The package to test (example: azure.machinelearning)')
    parser.add_argument('-t',
                        required=True,
                        help='The path to the tests. (example: tests/azure-machinelearning/unittests')
    args = parser.parse_args()

    # pytest --junitxml=junit1.xml --cov=azure.machinelearning --cov-report=html --cov-report=xml -ra ./tests/*/unittests/
    run_command([
        'pytest', '--junitxml=junit1.xml', f'--cov={args.p}', '--cov-report=html', '--cov-report=xml', '-ra',
