...
def load_data_params(self):
    with self.argument_context('ml data list') as c:
        add_common_params(c)
        c.argument('name', help="Name of the data asset. If name is None, all data assets of the workspace will be returned, otherwise all versions of the data asset with the specified name will be returned.")

    with self.argument_context('ml data show') as c:
        add_common_params(c)
        c.argument('name', help="Name of the data asset, required.")
        c.argument('version', help="Version of the data asset, required.")

    with self.argument_context('ml data create') as c:
        add_common_params(c)
        c.argument('name', help="Name of the data asset, required.")
        c.argument('version', help="Version of the data asset, only postive integer is allowed, required.")
    ...
    with self.argument_context('ml data upload') as c:
        ...
        c.argument('datastore', help="Datastore where the data asset will be uploaded to, default to the default datastore associated with the Azure ML workspace.")
