...
def ml_job_create(cmd,
    ...
    try:
        ...
        if str(err) is not None and "A command job's data binding cannot be changed." in str(err):
            print_error("(User error) A job with that name already exists. Please change the name and resubmit.")
        else:
            print_error(str(err))
