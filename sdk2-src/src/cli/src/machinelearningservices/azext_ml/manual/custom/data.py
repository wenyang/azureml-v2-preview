...
def ml_data_create(cmd, resource_group_name, workspace_name, name=None, version=None, file=None):
    subscription_id = get_subscription_id(cmd.cli_ctx)
...
def ml_data_update(cmd, resource_group_name, workspace_name, name=None, version=None, description=None, file=None):
    subscription_id = get_subscription_id(cmd.cli_ctx)
...
def ml_data_upload(cmd, resource_group_name, workspace_name, name, version, path, datastore=None, description=None):
    subscription_id = get_subscription_id(cmd.cli_ctx)
    ...
    return ml_client.data.upload(
        name=name, version=version, description=description, local_path=path, linked_service_name=datastore
    )
