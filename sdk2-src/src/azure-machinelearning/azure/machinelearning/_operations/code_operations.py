...
class CodeOperations(_WorkspaceDependentOperations):
    ...
    def create(self, name: str, directory: Union[str, os.PathLike], version: str = None,
               datastore_name: Optional[str] = None, show_progress: bool = True,
               _unit_test: bool = False) -> CodeVersionResource:
        ...
        if _unit_test:  # unittests timeout for upload step
            remote_path = resource_id = None
        else:
            remote_path, resource_id = artifact_utils.upload_artifact(local_path=directory,
                                                                      datastore=datastore,
                                                                      show_progress=show_progress)
            resource_id.format(self._workspace_scope.subscription_id,
