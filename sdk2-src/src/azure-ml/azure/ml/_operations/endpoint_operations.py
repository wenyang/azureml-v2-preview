...
from azure.ml._restclient.machinelearningservices.models import (
    OnlineEndpointPropertiesTrackedResource, OnlineEndpointPropertiesTrackedResourceArmPaginatedResult,
...
from azure.ml.constants import (API_VERSION_2020_12_01_PREVIEW, ONLINE_ENDPOINT_TYPE, BATCH_ENDPOINT_TYPE,
                                BASE_PATH_CONTEXT_KEY, API_VERSION_2020_09_01_PREVIEW, WORKSPACE_CONTEXT_KEY)
from azure.ml._schema._endpoint.online.online_endpoint import OnlineEndpointSchema, InternalOnlineEndpoint, InternalEndpoint
...
class EndpointOperations(_WorkspaceDependentOperations):
    ...
    def _get_workspace_location(self, workspace_name: str) -> str:
        return self._all_operations.all_operations["workspaces"].get(workspace_name).location
    ...
    def _create_online_endpoint(self,
        ...
        # create the endpoint
        endpoint_resource = internal_endpoint._to_rest_online_endpoint(location)
        ...
        for _, deployment in internal_endpoint.deployments.items():
            deployment_resource = deployment._to_rest_online_deployments(
                compute_type=internal_endpoint.cluster_type,
                location=self._get_workspace_location(self._workspace_name))
            self._online_deployment.create_or_update(
                subscription_id=self._subscription_id,
                resource_group_name=self._resource_group_name,
                workspace_name=self._workspace_name,
                endpoint_name=name or internal_endpoint.name,
                name=deployment.name,
                body=deployment_resource,
                api_version=API_VERSION_2020_12_01_PREVIEW,
                **self._init_kwargs)

        # set the traffic
        endpoint_resource_with_traffic = internal_endpoint._to_rest_online_endpoint_with_traffic(location)
        return self._online_operation.create_or_update(
            subscription_id=self._subscription_id,
            resource_group_name=self._resource_group_name,
            workspace_name=self._workspace_name,
            name=name or internal_endpoint.name,
            body=endpoint_resource_with_traffic,
            api_version=API_VERSION_2020_12_01_PREVIEW,
            **self._init_kwargs)
