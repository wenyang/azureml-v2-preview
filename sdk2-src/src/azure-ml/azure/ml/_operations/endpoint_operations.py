...
class EndpointOperations(_WorkspaceDependentOperations):
    ...
    def _create_online_endpoint(self,
        ...
        for deployment_name, deployment in internal_endpoint.deployments.items():
            deployment.code_configuration.arm_id = orchestrators.get_code_asset_arm_id(
                code_configuration=deployment.code_configuration)
            deployment.environment.arm_id = orchestrators.get_environment_arm_id(environment=deployment.environment)
            deployment.model.arm_id = orchestrators.get_model_arm_id(model=deployment.model)
