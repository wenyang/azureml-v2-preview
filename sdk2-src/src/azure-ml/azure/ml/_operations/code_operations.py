...
from azure.ml._utils._arm_id_utils import get_datastore_arm_id, try_get_arm_id
...
class CodeOperations(_WorkspaceDependentOperations):
    ...
    def _get_code_artifact_arm_id(self, code: str) -> str:
        # there are 4 cases for the format of code
        # 1. None (only the command is provided)
        # 2. _arm_id
        # 3. artifact name with version (version might be missing)
        # 4. local path(file or directory)
        if code:
            is_arm_id, arm_id = try_get_arm_id(code)
            if is_arm_id:
                return arm_id
            ...
            if path.exists():
                ...
                name = str(uuid.uuid4())
                code_artifact = self.create(name, str(path))
                return code_artifact.id

            return self.show(name=code).id
        return code
