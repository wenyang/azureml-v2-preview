...
from azure.ml._restclient.machinelearningservices.models import (AssetPath, ModelContainer, ModelContainerResource,
...
class ModelOperations(_WorkspaceDependentOperations):
    ...
    def _upload_to_datastore(self, path: Union[str, Path, os.PathLike]) -> Tuple[AssetPath, str]:
        self._validate_path(path)
        datastore_name = self._datastore_operations.get_default().name  # type: ignore
        asset_path = upload_artifact(str(path), self._datastore_operations, datastore_name, include_container_in_asset_path=True)
        datastore_resource_id = get_datastore_arm_id(datastore_name, self._workspace_scope)  # type: ignore
    ...
    def _parse_name_version(self, name: str) -> Tuple[str, Optional[int]]:
        # TODO: not a good place to define this func. Should be generalized.
    ...
    def _create_model(self, internal_model: InternalModel, name: str = None, version: int = None):
        ...
        asset_path, datastore_resource_id = self._upload_to_datastore(_asset_path)
        model_container_resource = self._model_container_operation.create_or_update(
