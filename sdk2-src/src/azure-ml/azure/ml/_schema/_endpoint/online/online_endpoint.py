...
from azure.ml._restclient.machinelearningservices.models import (
    ...
    MirComputeConfiguration, ManagedIdentity)
from azure.ml.constants import ARM_ID_FULL_PREFIX, ONLINE_ENDPOINT_TYPE, BASE_PATH_CONTEXT_KEY, AKS, MIR, ACI, MANAGED, \
    KEY
...
class InternalOnlineEndpoint(InternalEndpoint):
    ...
    def _to_rest_compute_configuration(self) -> Tuple[ComputeConfiguration, ManagedIdentity]:
        if self.cluster_type == MIR:
            return MirComputeConfiguration(), ManagedIdentity(type="SystemAssigned")
        elif self.cluster_type == ACI:
            return AciComputeConfiguration(), ManagedIdentity(type="None")
        else:
    ...
    def _to_rest_online_endpoint_with_traffic(self, location: str) -> OnlineEndpointPropertiesTrackedResource:
        ...
        return OnlineEndpointPropertiesTrackedResource(
            location=location, properties=properties, identity=identity)
    ...
    def _get_infra_from_rest(self, compute_config: ComputeConfiguration):
        if isinstance(compute_config, AksComputeConfiguration):
...
class OnlineEndpointSchema(EndpointSchema):
    ...
    def make(self, data: Any, **kwargs: Any) -> InternalOnlineEndpoint:
        ...
        if infra == MANAGED:
            ...
            cluster_type = AKS
        if not data.get("auth_mode", None) and (cluster_type == MIR or cluster_type == AKS):
            data["auth_mode"] = KEY
        deployments = data.get("deployments", None)
        if deployments:
            for name, deployment in deployments.items():
                if not deployment.sku and cluster_type == MIR:
