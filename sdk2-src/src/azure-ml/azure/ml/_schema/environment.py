...
from marshmallow import fields, post_load, validate, pre_dump, ValidationError
from typing import Optional, Dict
from .schema import YamlFileSchema, PatchedNested, PatchedSchemaMeta
...
class DockerSchema(metaclass=PatchedSchemaMeta):
    image = fields.Str()
    build = fields.Dict(keys=fields.Str(validate=validate.OneOf(["dockerfile"])))
...
class EnvironmentSchema(YamlFileSchema):
    ...
    def validate(self, data, **kwargs):
        ...
        if name is None or version is None:
            raise ValidationError("Environment must specify name and version")
        return data
...
class InternalDocker:
    def __init__(self, base_path: Optional[str] = None, *, image: Optional[str] = None, build: Optional[Dict] = None):
        self._image = image
        self._build = build
    ...
    def translate_to_rest_object(self) -> EnvironmentSpecificationVersionResource:
        ...
        if self.docker is not None:
            if self.docker.image is not None:
                docker_image = DockerImage(docker_image_uri=self.docker.image)
                properties.update({"docker": docker_image})
            if self.docker.build is not None:
                docker_build = DockerBuild(dockerfile=load_file(Path(base_path, self.docker.build.get("dockerfile"))))
