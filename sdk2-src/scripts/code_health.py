...
def test(rootpath, testpath):
    ...
    for path in custom:
        ...
        flake_rules["exclude"] = [
            os.path.dirname(a)
            for a in glob.glob(path + "/**/flake_rules.json", recursive=True)
            if os.path.dirname(a) != path
        ]
        inherited_rules = inherit_flake_rules(testpath, path)
        flake_rules = combine_rules(
            combine_rules(flake_rules, test_path_flake_rules), combine_rules(inherited_rules, load_rules(path))
        )
...
if __name__ == "__main__":
    ...
    print(args)
    main(Path(args.testpath).resolve())
