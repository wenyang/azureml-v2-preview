...
{
    Indent
    $__file = Get-ChildItem -Path $source -Filter "$package\dist\*.whl" | Select-Object -First 1

	Write-Host $__file
    $__blob = [string]::Concat($prefix, $__file)

    $__text = Get-Content "$source\$package\dist\$__file" -Raw -Encoding Byte
    ...
    {
        $supress = Set-AzureStorageBlobContent -Container $storage.container -File "$source\$package\dist\$__file" -Blob $__blob -Force
    }
    UnIndent
}
