...
setup sdk, cli with debug and verbose mode:                 {Color.BLUE} python scripts/dev_setup -scdv {Color.END}
    ...
    if args.wheel:
        ...
        run_command(commands=['python', 'scripts/build_wheel.py', '-s', './src/azure-ml', '-v', './src/azure-ml/azure/ml'], **command_args)
        run_command(commands=['python', 'scripts/build_wheel.py', '-s', './src/cli/src/machinelearningservices', '-v', './src/cli/src/machinelearningservices/azext_ml/manual'], **command_args)
    if args.clean:
