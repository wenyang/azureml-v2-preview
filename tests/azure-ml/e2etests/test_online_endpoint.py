...
def debug_client() -> MLClient:
    ...
    resource_group_name = "chpirill"
    workspace_name = "chpirillcentral"
...
@pytest.mark.e2etest
@pytest.mark.skip(reason="bug on MFE endpoint backend.")
def test_online_endpoint_aks_e2e(endpoint_aks_yaml: str, client: MLClient) -> None:
    ...
    run_endpoint_tests_e2e(endpoint_yaml=endpoint_aks_yaml, client=client)


@pytest.mark.skip(reason="MIR test is not availabe on the backend")
@pytest.mark.e2etest
def test_online_endpoint_mir_e2e(endpoint_mir_yaml: str, client: MLClient) -> None:
    run_endpoint_tests_e2e(endpoint_yaml=endpoint_mir_yaml, client=client)


def run_endpoint_tests_e2e(endpoint_yaml: str, client: MLClient) -> None:
    obj = None
    try:
        with open(endpoint_aks_yaml, 'r') as f:
        ...
        assert get_obj["kind"] == obj.kind
        assert get_obj["infrastructure"] == obj.infrastructure
        ...
        # disable the list test since currently the workspace has too many endpoints and list hangs
