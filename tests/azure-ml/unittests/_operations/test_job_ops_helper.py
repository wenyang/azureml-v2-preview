...
class TestJobLogManager:
    ...
    def test_wait_for_completion_with_output(self, mock_run_operations):
        dummy_job = DummyJob()
        with patch.object(RunOperations, 'get_run_details', side_effect=[
            RunDetailsDto(status="Finalizing", log_files={
                "log1": "Log",
                "log2": "log"
            }), RunDetailsDto(status="Completed", log_files={
                "log1": "Log",
                "log2": "log"
            })]) as get_run_mock:
    ...
    def test_wait_for_completion_with_error_silent(self, mock_run_operations):
        dummy_job = DummyJob()
        with patch.object(RunOperations, 'get_run_details', return_value=RunDetailsDto(status="Failed", warnings=[RunDetailsWarningDto(message="bad luck")])) as get_run_mock:
    ...
    def test_wait_for_completion_with_error_raise(self, mock_run_operations):
        dummy_job = DummyJob()
        with patch.object(RunOperations, 'get_run_details', return_value=RunDetailsDto(status="Failed")) as get_run_mock:
