...
def generate_asset_id():
    return AZ_ML_ARTIFACT_DIRECTORY + str(uuid.uuid4())
...
class StorageClient:
    ...
    def upload(self, source: str, show_progress: bool = True) -> str:
        ...
        full_dest_path = "/".join((self.container, asset_id))
        msg = f"Uploading {source} to {full_dest_path}"

        if os.path.isdir(source):
            self.upload_dir(source, asset_id, msg, show_progress)
        else:
            self.upload_file(source, asset_id, msg, show_progress)

        if not self.blob_exists(blob_name=asset_id):
            raise Exception("Upload was unsuccessful.")
