...
def ml_datastore_detach(cmd, resource_group_name, workspace_name, datastore_name):
    subscription_id = get_subscription_id(cmd.cli_ctx)
...
def ml_datastore_show(cmd, resource_group_name, workspace_name, datastore_name, include_secrets=False):
    subscription_id = get_subscription_id(cmd.cli_ctx)

    ml_client = MLClient(
        subscription_id=subscription_id, resource_group_name=resource_group_name, default_workspace_name=workspace_name
    )
    return ml_client.datastores.show(datastore_name, include_secrets=include_secrets)


def ml_datastore_list(
    cmd,
    resource_group_name,
    workspace_name,
