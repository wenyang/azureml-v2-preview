...
def get_storage_client(credential: str, container_name: str, storage_account: str,
                       storage_type: str) -> Union[DefaultStorageClient, FileStorageClient]:
    """
    ...
    if storage_type in ["AzureBlob", "AzureDataLakeGen2"]:
        return DefaultStorageClient(credential=credential, container_name=container_name,
                                    account_url=account_url)
    elif storage_type == "AzureFile":
        return FileStorageClient(credential=credential, file_share_name=container_name,
                                 account_url=account_url)
