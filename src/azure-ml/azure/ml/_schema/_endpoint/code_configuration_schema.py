...
from azure.ml._schema.code_asset import CodeAssetSchema, InternalCodeAsset
...
class InternalCodeConfiguration:
    ...
    def _to_rest_code_configuration(self) -> CodeConfiguration:
        return CodeConfiguration(code_artifact_id=self.code, command=self.scoring_script)
    ...
    def _from_rest_code_configuration(code_configuration: CodeConfiguration):
        if code_configuration:
            return InternalCodeConfiguration(
                code=code_configuration.code_artifact_id, scoring_script=code_configuration.command[0]
            )
