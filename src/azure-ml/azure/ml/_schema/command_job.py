...
from .base import PatchedNested, PathAwareSchema
from .compute_binding import ComputeBindingSchema
from .environment import InternalEnvironment
from .code_asset import check_or_create_code_asset
...
class InternalCommandJob:
    ...
    def upload_dependencies(self, code_operations: "CodeOperations"):
        ...
        self.run.code = check_or_create_code_asset(self.run.code, self._base_path, code_operations)
    ...
    def generate_code_configuration(self) -> CodeConfiguration:
        return CodeConfiguration(command=self.run.command.split(), code_artifact_id=self.run.code)
