...
import re
from .input_entry import InputEntrySchema, InputEntry
...
class InternalCommandJob:
    ...
    def unbind_inputs(self, properties) -> None:
        ...
        for key, input in properties.data_bindings.items():
            ...
            inputs[key] = InputEntry(mode=input.mode, name=input.source_data_reference)
        self.inputs = inputs
    ...
    def serialize_from_rest_object(self, obj: CommandJobResource):
        ...
        self.unbind_inputs(properties)
        self._compute = properties.compute_binding
        self.name = obj.name
        self.properties = properties.properties
        self.experiment_name = properties.experiment_name
        self._code_arm_id = properties.code_configuration.code_artifact_id
        ...
        return CommandJobSchema(context={BASE_PATH_CONTEXT_KEY: "./"}).dump(self)    # type: ignore
