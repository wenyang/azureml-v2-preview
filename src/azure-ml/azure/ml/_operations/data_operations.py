...
class DataOperations(_WorkspaceDependentOperations):
    ...
    def list(self, name: str = None) -> Iterable[DataVersionResourceArmPaginatedResult]:
        name, version = _parse_name_version(name, version_as_int=False)
    ...
    def upload(self,
        ...
        asset_path = upload_artifact(path, self._datastore_operations, datastore_name, include_container_in_asset_path=False)
        datastore_resource_id = get_datastore_arm_id(datastore_name, self._workspace_scope)
        if is_directory:
            return self.create(name=name, description=description, linked_service_id=datastore_resource_id, directory_path=asset_path.path)
        else:
            return self.create(name=name, description=description, linked_service_id=datastore_resource_id, file_path=asset_path.path)
