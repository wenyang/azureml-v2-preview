...
class DataOperations(_WorkspaceDependentOperations):
    def __init__(self, workspace_scope: WorkspaceScope,
        ...
        self._container_operation = service_client.data_container
    ...
    def list(self, name: str = None) -> List[InternalAsset]:
        if name:
            return list(
                map(lambda data_version: InternalAsset._from_data_version(str(name), data_version),
                    self._operation.list(name,
            ...
            return list(
                map(lambda data_container: InternalAsset._from_data_container(data_container),
                    self._container_operation.list(self._subscription_id,
                                                   self._resource_group_name,
                                                   self._workspace_name,
                                                   API_VERSION_2020_09_01_PREVIEW,
                                                   skip_token=None,
    ...
    def get(self, name: str, version: int) -> InternalAsset:
        data_version = self._operation.get(name,
                                           str(version),
                                           self._subscription_id,
                                           self._resource_group_name,
                                           self._workspace_name,
    ...
    def create_or_update(self,
                         ...
                         yaml_path: Union[str, os.PathLike, None] = None) -> InternalAsset:
        ...
        if loaded_data.name is None or loaded_data.version is None:
            raise Exception("Name and version are required, please provide name and version in \"<name>:<version>\" format or in yaml file.")
        ...
        result = self._operation.create_or_update(str(loaded_data.name),
    ...
    def upload(self,
               ...
               description: str = None,
               linked_service_name: str = None) -> InternalAsset:

        path = Path(local_path)
        is_directory = path.is_dir()

        datastore_name = linked_service_name or self._datastore_operations.get_default().name
        asset_path = upload_artifact(str(path), self._datastore_operations, datastore_name, include_container_in_asset_path=False)
        datastore_resource_id = get_datastore_arm_id(str(datastore_name), self._workspace_scope)
        if is_directory:
            return self.create_or_update(name=name, version=version, description=description, linked_service_id=datastore_resource_id, directory_path=asset_path.path)
        else:
            return self.create_or_update(name=name, version=version, description=description, linked_service_id=datastore_resource_id, file_path=asset_path.path)
    ...
    def _load_yaml(self, file: Union[str, os.PathLike, None] = None) -> InternalAsset:
        ...
        try:
            internal_asset: InternalAsset = AssetSchema(
                context={BASE_PATH_CONTEXT_KEY: base_path,
                         WORKSPACE_CONTEXT_KEY: self._workspace_scope}).load(cfg, unknown=RAISE)  # type: ignore
