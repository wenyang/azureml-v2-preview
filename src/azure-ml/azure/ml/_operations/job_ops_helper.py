...
def stream_logs_until_completion(
    run_operations: RunOperations, job_resource: JobBaseResource, raise_exception_on_failed_job=True
) -> None:
    ...
    try:
        ...
        while (
            _current_details[STATUS_KEY] in RunHistoryConstants.IN_PROGRESS_STAUSES
            or _current_details[STATUS_KEY] == JobStatus.FINALIZING
        ):
            file_handle.flush()
            ...
            if current_log:
                content = download_text_from_url(
                    _current_details["logFiles"][current_log],
                    session,
                ...
                # TODO: Temporary solution to wait for all the logs to be printed in the finalizing state.
                if (
                    _current_details[STATUS_KEY] not in RunHistoryConstants.IN_PROGRESS_STAUSES
                    and _current_details[STATUS_KEY] == JobStatus.FINALIZING
