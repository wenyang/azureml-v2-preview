...
class ModelOperations(_WorkspaceDependentOperations):
    ...
    def create(self,
               ...
               **kwargs: Any) -> ModelVersionResource:  # TODO: Are we going to implement job_name?
        internal_model = self._load(file, **kwargs)
        return self._create_from(internal_model=internal_model)

    def show(self, name: str, version: int) -> ModelVersionResource:  # name:latest
    ...
    def list(self, name: str = None) -> Iterable[ModelVersionResourceArmPaginatedResult]:
        if name:
            return self._model_versions_operation.list(name=name, workspace_name=self._workspace_name, **self._scope_kwargs)
        else:
            return cast(Iterable[ModelVersionResourceArmPaginatedResult],
                        self._model_container_operation.list(workspace_name=self._workspace_name, **self._scope_kwargs))

    def _load(self, file: Union[str, os.PathLike, None],
              **kwargs: Any) -> InternalModel:
        cfg = load_yaml(file)
        context = {
